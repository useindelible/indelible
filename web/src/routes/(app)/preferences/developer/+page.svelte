<script lang="ts">
	...
	const ISSUE_BASELINE = {
		...
		scopes: ['read'] as ScopeKey[],
		expiry: '90' as ExpiryOption
	};
	...
	let issueName = $state(ISSUE_BASELINE.name);
	let issueScopes: Set<ScopeKey> = new SvelteSet(ISSUE_BASELINE.scopes);
	let issueExpiry = $state<ExpiryOption>(ISSUE_BASELINE.expiry);
	...
	const isDirty = $derived(
		issueName !== ISSUE_BASELINE.name ||
			!setsEqual(issueScopes, new SvelteSet(ISSUE_BASELINE.scopes)) ||
			issueExpiry !== ISSUE_BASELINE.expiry ||
			addEvents.size > 0 ||
			addName.length > 0 ||
			addUrl.length > 0 ||
			!addActive
	...
	onMount(() => {
		...
		const requestedScope = page.url.searchParams.get('scope');
		if (requestedScope && SCOPE_DEFS.some((scope) => scope.key === requestedScope)) {
			issueScopes.clear();
			issueScopes.add(requestedScope as ScopeKey);
			issueName = 'Obsidian plugin';
			issueOpen = true;
		}
	});
	...
	function toggleIssueScope(scope: ScopeKey) {
		if (issueScopes.has(scope)) issueScopes.delete(scope);
		else issueScopes.add(scope);
	}

	async function submitIssueToken() {
		if (issueScopes.size === 0) {
			issueError = 'Pick at least one scope.';
			return;
		}
		...
		creatingToken = true;
		issueError = null;
		const expiresIn = issueExpiry === 'never' ? null : Number(issueExpiry) * 24 * 60 * 60;
		const res = await createApiToken({
			name: issueName.trim(),
			scopes: Array.from(issueScopes),
	...
	function discardIssueForm() {
		issueName = ISSUE_BASELINE.name;
		issueScopes.clear();
		for (const scope of ISSUE_BASELINE.scopes) issueScopes.add(scope);
		issueExpiry = ISSUE_BASELINE.expiry;
	}
...
<div class="page">
	...
	<div class="settings-body">
		<ApiTokenPanel
			...
			onCloseIssue={() => (issueOpen = false)}
			onIssueName={(name) => (issueName = name)}
			onToggleIssueScope={toggleIssueScope}
