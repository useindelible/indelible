...
export function formatRelativeTime(iso: string | null | undefined): string {
	return relativeTime(iso) ?? get(t)('reader_never');
}
...
export function formatSchedule(minutes: number | null | undefined): string {
	if (!minutes) return get(t)('feed_management_schedule_default');
	if (minutes < 60) return get(t)('feed_management_schedule_minutes', { values: { minutes } });
	const hours = Math.round(minutes / 60);
	if (hours < 24) return get(t)('feed_management_schedule_hours', { values: { hours } });
	const days = Math.round(hours / 24);
	return get(t)('feed_management_schedule_days', { values: { days } });
}
