...
let triageMode = $state<TriageModeDto>('manual');
...
async function fetchPage(): Promise<void> {
	...
	try {
		// Smart-list views evaluate ad-hoc instead of the saved-id endpoint so the page's
		// item_type scope (Articles/Emails/...) composes with the list's own expression.
		const body =
			smartListId && activeSmartList
				? buildSmartListItemsQueryBody({
						filterExpression: fromApiFilterExpression(activeSmartList.filter_expression),
						cursor,
						limit: PAGE_LIMIT
					})
	...
	if (id) {
		try {
			...
			if (resp.data) {
				...
				// Don't overwrite conditions the user is actively editing in the filter bar
				if (!filterBarOpen) {
...
function getDraftFilterExpression(): FilterExpression {
	return buildFilterExpression([...draftConditions], draftConjunction);
}
