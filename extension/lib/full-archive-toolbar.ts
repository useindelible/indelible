...
export interface ToolbarState {
  view: 'checking' | 'saving' | 'saved' | 'disconnected' | 'unsupported' | 'error' | 'already-saved'
  serverUrl?: string
  readerUrl?: string
  url?: string
  step?: string
...
function bindToolbarEvents(root: ShadowRoot, state: ToolbarState): void {
  ...
  const doConnect = (): void => {
    ...
    void browser.runtime.sendMessage({
      ...
      serverUrl: input?.value.trim() || 'https://useindelible.com',
    })
  }
  root.querySelector('[data-action="connect"]')?.addEventListener('click', doConnect)
  root
