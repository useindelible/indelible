...
export default defineBackground(() => {
  ...
  // Watch for the auth callback redirect
  browser.tabs.onUpdated.addListener(async (tabId, changeInfo) => {
    if (changeInfo.url) {
      ...
      const callbackPrefix = `${serverUrl}/extension/auth/callback`

      if (changeInfo.url.startsWith(callbackPrefix)) {
        try {
...
async function handleAuthStart(
  ...
  try {
    ...
    await connect(serverUrl, returnTabId)
    return { success: true }
  } catch (err) {
    return { success: false, error: String(err) }
  }
