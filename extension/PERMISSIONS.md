...
| `identity`     | Open the server's interactive Authorization Code + PKCE consent flow and let the browser capture the registered callback. The extension never receives the user's Indelible password.                                                                                               |
...
Chrome, Edge, and Firefox generate a callback owned by the browser through
`identity.getRedirectURL('indelible')`. Indelible opens the consent flow with
`identity.launchWebAuthFlow()` and the browser closes that window when the exact
callback is reached. Authentication does not open a normal tab and does not
monitor unrelated tab navigation.
