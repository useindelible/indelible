...
- `identity` runs the interactive Authorization Code + PKCE flow and captures the browser-owned callback.
- `tabs` is used for active-tab URL/title metadata and resuming a save after sign-in.
- `storage` stores the refresh token, configured server origin, connection timestamp, and short-lived PKCE state.

Authentication does not open or monitor a normal browser tab. See
`STORE_REVIEW.md` for the callback and store-identity contract.
