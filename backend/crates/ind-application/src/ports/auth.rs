...
use ind_domain::{
    ApiPermission, ApiToken, ApiTokenId, ClientType, OAuthProvider, RefreshToken, Theme, User,
    UserId,
};
...
pub enum AuthError {
    ...
    /// The operation depends on sending mail, and this deployment has no
    /// outbound transport configured to send it.
    #[error("mail delivery is not configured")]
    MailTransportUnavailable,
}
