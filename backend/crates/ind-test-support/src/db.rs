...
const SILO_USER: &str = "siloadmin";
const SILO_PASSWORD: &str = "siloadmin";
const SILO_IMAGE: &str = "pgsty/silo";
const SILO_TAG: &str = "RELEASE.2026-08-06T00-00-00Z";
...
struct SharedContainers {
    ...
    silo_container: StdMutex<Option<ContainerAsync<GenericImage>>>,
...
/// without this the shared Postgres/Silo containers outlive the test binary and
...
fn cleanup_shared_containers() {
    ...
    let silo = shared.silo_container.lock().ok().and_then(|mut g| g.take());
    if pg.is_none() && silo.is_none() {
        return;
    }
...
impl SharedContainers {
            ...
            .get_or_init(|| {
                ...
                std::thread::spawn(|| {
                    ...
                    Arc::new(SharedContainers {
                        ...
                        silo_container: StdMutex::new(Some(shared.1)),
                        runtime,
                        admin_pool: shared.2,
                        pg_host: shared.3,
                        pg_port: shared.4,
                        silo_endpoint: shared.5,
    ...
    async fn init_async() -> (
        ...
        let (silo_container, silo_endpoint) = Self::start_silo().await;
        ...
        Self::create_silo_bucket(&silo_endpoint).await;
    ...
    async fn start_silo() -> (ContainerAsync<GenericImage>, String) {
        let image = GenericImage::new(SILO_IMAGE, SILO_TAG)
            .with_exposed_port(ContainerPort::Tcp(9000))
            .with_wait_for(WaitFor::Log(LogWaitStrategy::stderr("API:")));

        let container = image
            .with_env_var("MINIO_ROOT_USER", SILO_USER)
            .with_env_var("MINIO_ROOT_PASSWORD", SILO_PASSWORD)
            .with_cmd(["server", "/data"])
            .with_startup_timeout(std::time::Duration::from_secs(60))
            .start()
            .await
            .expect("failed to start silo container");

        // Silo logs "API:" (the wait condition) a moment before Docker reliably
        ...
        let port = silo_port.expect("failed to get silo mapped port after retries");

        let host = container
            .get_host()
            .await
            .expect("failed to get silo container host");
    ...
    async fn create_silo_bucket(endpoint: &str) {
        Self::s3_client(endpoint)
            ...
            .send()
            .await
            .expect("failed to create silo test bucket");
    }

    async fn s3_client(endpoint: &str) -> aws_sdk_s3::Client {
        let credentials = Credentials::new(SILO_USER, SILO_PASSWORD, None, None, "test");
    ...
    async fn build_s3_client(&self) -> S3Client {
        S3Client::from_client(
            Self::s3_client(&self.silo_endpoint).await,
