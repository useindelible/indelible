...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Asset metadata with an API-origin download URL", body = crate::routes::documents::DocumentAssetResponse),
...
pub async fn extension_get_entry_asset(
    ...
    let joined = library_entry_for_alias(&state, auth_user.user_id, entry_id).await?;
    let asset = document_reader_ops
        .get_completed_asset(auth_user.user_id, joined.document.id, kind)
        .await
        .map_err(ApiError::from)?;
    Ok(ApiResponse::new(
        crate::routes::documents::DocumentAssetResponse::from_asset(asset, &state.config.base_url),
    ))
}
