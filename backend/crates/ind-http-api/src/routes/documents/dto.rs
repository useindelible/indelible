...
use ind_application::ports::{DocumentAssetWithUrl, DocumentReaderView};
    ...
    pub fn from_view(
        ...
        let thumbnail_url = view.document.thumbnail_url.or_else(|| {
            view.assets
                ...
                .then(|| {
                    format!(
                        "{asset_base_url}/api/v1/assets/documents/{}/thumbnail",
                        view.document.id
                    )
                })
        });
...
/// Document asset metadata plus a presigned download URL for its bytes (the reader fetches the
/// readable HTML from this URL).
#[derive(Debug, Serialize, ToSchema)]
...
impl From<DocumentAssetWithUrl> for DocumentAssetResponse {
    fn from(value: DocumentAssetWithUrl) -> Self {
