...
use crate::middleware::{
    RequireAiRead, RequireAiReadAndLibraryRead, RequireAiUseAndLibraryRead, RequireAiWrite,
    RequireAiWriteAndAiUse,
};
...
#[utoipa::path(
    ...
    security(("bearer" = []), ("api_token" = [])),
    extensions(("x-indelible-permissions" = json!(["ai:write", "ai:use"]))),
    tag = "TTS",
)]
