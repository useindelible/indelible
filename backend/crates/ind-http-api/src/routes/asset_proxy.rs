...
use crate::middleware::{AvatarAssetAccess, DocumentAssetAccess};
use crate::state::AppState;

const PRESIGNED_REDIRECT_EXPIRY: Duration = Duration::from_secs(3600);

/// In `presigned` mode the proxy answers with a redirect to a presigned URL
/// instead of streaming, offloading asset bandwidth to the object store. This
/// is the only place allowed to presign: response bodies always carry API
/// URLs, so an operator whose S3 endpoint is not browser-reachable (the
/// shipped compose default) can never receive a dead URL in a payload.
async fn presigned_redirect(storage: &dyn ObjectStorage, key: &str) -> Result<Response, ApiError> {
    let url = storage
        .presigned_url(key, PRESIGNED_REDIRECT_EXPIRY)
        .await
        .map_err(ApiError::from)?;
    let response = Response::builder()
        .status(StatusCode::FOUND)
        .header(http::header::LOCATION, &url)
        // Signed URLs expire; never let intermediaries cache the redirect.
        .header(http::header::CACHE_CONTROL, "private, no-store")
        .body(Body::empty())
        .map_err(|error| ApiError::Internal {
            message: format!("failed to build redirect response: {error}"),
        })?
        .into_response();
    Ok(response)
}
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 302, description = "Redirect to a presigned URL (presigned mode)"),
        (status = 401, description = "Authentication required"),
        (status = 403, description = "User access JWT or signed asset cookie required"),
        (status = 404, description = "Avatar not found or not owned by caller"),
        (status = 503, description = "Storage not configured"),
    ),
    security(("bearer" = []), ("asset_cookie" = [])),
    tag = "Asset Proxy",
)]
pub async fn stream_avatar(
    asset_access: AvatarAssetAccess,
    ...
    if state.config.asset_serving_mode == AssetServingMode::Presigned {
        return presigned_redirect(storage.as_ref(), &avatar_key).await;
    }
...
pub async fn stream_document_asset(
    asset_access: DocumentAssetAccess,
    State(state): State<AppState>,
    ...
    let asset = ops
        .get_completed_asset(asset_access.user_id, document_id, kind)
        .await
        .map_err(ApiError::from)?;

    if state.config.asset_serving_mode == AssetServingMode::Presigned {
        return presigned_redirect(storage.as_ref(), &asset.s3_key).await;
    }
    ...
    let object_data = storage
        ...
        .await
        .map_err(ApiError::from)?;
    build_stream_response_with_kind(object_data, Some(asset.asset_kind))
}
