...
use crate::middleware::RequireObsidianPluginScope;
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 403, description = "obsidian_plugin scope required"),
    ),
    security(("api_token" = [])),
    tag = "Export",
)]
pub async fn create_obsidian_run(
    RequireObsidianPluginScope(auth_user): RequireObsidianPluginScope,
