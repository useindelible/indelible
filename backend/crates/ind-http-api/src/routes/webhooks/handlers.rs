...
#[utoipa::path(
    ...
    extensions(("x-indelible-permissions" = json!(["webhooks:read"]))),
    tag = "Webhooks",
)]
...
#[utoipa::path(
    ...
    security(("bearer" = []), ("api_token" = [])),
    extensions(("x-indelible-permissions" = json!(["webhooks:write"]))),
    tag = "Webhooks",
)]
pub async fn create_webhook_endpoint(
    RequireWebhooksWrite {
        principal: auth_user,
        ..
    }: RequireWebhooksWrite,
    State(state): State<AppState>,
...
#[utoipa::path(
    ...
    params(("webhook_id" = String, Path, description = "Webhook endpoint ID with whk_ prefix")),
    responses(
        (status = 200, description = "Webhook test attempt completed; inspect outcome for delivery result", body = WebhookDeliveryResponse),
        (status = 401, description = "Authentication required"),
        (status = 403, description = "Email verification required"),
        (status = 404, description = "Webhook endpoint not found"),
        (status = 422, description = "Validation error"),
