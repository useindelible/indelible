...
pub fn non_empty_token_permissions<T>(value: &[T]) -> Result<(), ValidationError> {
    if value.is_empty() {
        Err(error_with_message(
            "length",
...
pub fn valid_api_token_expiry(value: &chrono::Duration) -> Result<(), ValidationError> {
    if *value <= chrono::Duration::zero()
        || value.num_seconds() > crate::MAX_API_TOKEN_EXPIRY_SECONDS
    {
        return Err(error_with_message(
            "range",
            format!(
                "must be between 1 and {} seconds",
                crate::MAX_API_TOKEN_EXPIRY_SECONDS
            ),
        ));
    }
...
mod tests {
    ...
    fn text_and_collection_boundaries_are_enforced() {
        ...
        assert!(non_empty_token_permissions(&["library:read"]).is_ok());
        assert!(non_empty_token_permissions::<&str>(&[]).is_err());
        assert!(valid_api_token_expiry(&chrono::Duration::seconds(1)).is_ok());
        assert!(
            valid_api_token_expiry(&chrono::Duration::seconds(
                crate::MAX_API_TOKEN_EXPIRY_SECONDS
            ))
            .is_ok()
        );
        assert!(valid_api_token_expiry(&chrono::Duration::zero()).is_err());
        assert!(
            valid_api_token_expiry(&chrono::Duration::seconds(
                crate::MAX_API_TOKEN_EXPIRY_SECONDS + 1
            ))
            .is_err()
        );
    }
