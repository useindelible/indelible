...
#[tokio::test]
async fn reasoning_capable_article_chat_omits_sampling_parameters() {
    let h = ChatHarness::new(MilaSessionType::SingleDocument);
...
#[tokio::test]
async fn sampling_article_chat_keeps_sampling_parameters() {
    let h = ChatHarness::new(MilaSessionType::SingleDocument);
    h.config.lock().unwrap().supports_reasoning_effort = false;

    h.run().await;

    let requests = h.provider.chat_requests.lock().unwrap();
    assert_eq!(requests[0].temperature, Some(0.3));
    assert_eq!(requests[0].top_p, Some(0.95));
    let json = serde_json::to_value(&requests[0]).unwrap();
    assert!(json.get("reasoning_effort").is_none());
}
