...
pub fn validate_url(
    ...
    match url.host() {
        ...
        Some(Host::Domain(domain)) => {
            if (is_localhost_domain(domain) || is_docker_internal_domain(domain))
...
fn host_is_local_or_private(url: &Url) -> bool {
    match url.host() {
        Some(Host::Domain(d)) => is_localhost_domain(d) || is_docker_internal_domain(d),
...
/// Docker's magic host names resolve to the container host's gateway; they are
/// as unambiguously private as `localhost` and never route to the public
/// internet, so they share localhost's treatment on both the scheme and the
/// private-target gates.
fn is_docker_internal_domain(domain: &str) -> bool {
    let domain = domain.trim_end_matches('.');
    domain.eq_ignore_ascii_case("host.docker.internal")
        || domain.eq_ignore_ascii_case("gateway.docker.internal")
}
