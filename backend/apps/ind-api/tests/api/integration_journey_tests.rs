...
#[tokio::test]
async fn obsidian_journey_provisions_scoped_access_and_previews_settings() {
    ...
    let token = response(
        client
            .post_json(
                "/api/v1/tokens",
                &json!({"name": "Obsidian", "scopes": ["obsidian_plugin"]}),
            )
            .await,
        StatusCode::CREATED,
    )
    ...
    assert_eq!(token["scopes"], json!(["obsidian_plugin"]));
    ...
    assert_status(
        client
            .post_json("/api/v1/export/obsidian/runs", &json!({}))
            .await,
