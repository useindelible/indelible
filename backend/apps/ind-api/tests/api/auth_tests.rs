...
#[tokio::test]
async fn permissioned_api_token_lifecycle_crosses_route_middleware_and_persistence() {
    let app = TestApp::new().await;
    let session = app.create_web_session().await;
    ...
    let created = account
        .post_json(
            "/api/v1/tokens",
            &serde_json::json!({"name": "automation", "permissions": ["library:read"]}),
        )
        .await;
...
async fn account_profile_password_and_deletion_mutations_preserve_the_public_contract() {
    ...
    // Seed owned content so deletion proves erasure, not just account state.
    let saved = ind_test_support::factories::SavedDocumentFactory::new(session.user.id)
        .insert(app.pool())
        .await;
    ...
    let users_left: i64 = sqlx::query_scalar("SELECT count(*) FROM users WHERE id = $1")
        .bind(session.user.id.into_uuid())
        .fetch_one(app.pool())
        .await
        .unwrap();
    assert_eq!(users_left, 0, "delete forever must remove the account row");

    let docs_left: i64 = sqlx::query_scalar("SELECT count(*) FROM documents WHERE id = $1")
        .bind(saved.document_id.into_uuid())
        .fetch_one(app.pool())
        .await
        .unwrap();
