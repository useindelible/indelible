...
#[tokio::test]
async fn avatar_round_trip_uses_the_presigned_storage_contract() {
    let app = spawn_app().await;
    ...
    let upload = assert_json_response(
        client
            .post_json(
    ...
    .await;
    let upload_url = upload["upload_url"].as_str().expect("upload URL");
    let object_url = upload["object_url"].as_str().expect("object URL");
    let stored = app
        .client()
        .put(upload_url)
    ...
    assert!(stored.status().is_success(), "upload failed: {stored:?}");

    let profile = assert_json_response(
        client
            .patch_json("/api/v1/me", &json!({"avatar_url": object_url}))
            .await,
        StatusCode::OK,
    )
    .await;
    let read_url = profile["avatar_url"]
        .as_str()
        .expect("presigned avatar URL");
    assert_ne!(read_url, object_url);
    let downloaded = app
        .client()
        .get(read_url)
        .send()
        .await
        .expect("download avatar");
    assert_eq!(downloaded.status(), StatusCode::OK);
    assert_eq!(
        downloaded.bytes().await.expect("avatar bytes"),
        avatar.as_slice()
    );
    let proxied = client.get(&format!("/api/v1/assets/{object_url}")).await;
    ...
    assert_eq!(
        app.authed_client(&stranger)
            .get(&format!("/api/v1/assets/{object_url}"))
            .await
            .status(),
        StatusCode::NOT_FOUND
    );
