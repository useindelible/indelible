...
services:
  ...
  minio-init:
    ...
    entrypoint: >
      /bin/sh -c "
      mc alias set minio http://minio:9000 indelible ${MINIO_ROOT_PASSWORD} &&
...
Already running Postgres and S3? Drop the `postgres`, `minio`, and `minio-init`
...
- `S3_ENDPOINT`, `S3_BUCKET`, `S3_ACCESS_KEY`, `S3_SECRET_KEY`, and `S3_REGION`
  accept any S3-compatible provider. Set `S3_FORCE_PATH_STYLE=true` for MinIO,
...
its own origin, which needs no publicly reachable object store: MinIO stays
