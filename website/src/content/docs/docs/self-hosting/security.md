...
- `ASSET_SERVING_MODE=presigned` hands browsers short-lived URLs pointing directly
  at `S3_ENDPOINT`. Only use it when that endpoint is reachable from your users'
  browsers and terminates TLS. Pointing it at an internal address such as
  `http://minio:9000` yields links no browser can open.
