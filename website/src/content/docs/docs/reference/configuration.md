...
| Variable | Required | Description |
| --- | --- | --- |
| `ASSET_SERVING_MODE` | no | How archived assets are served: `passthrough` (default) streams them through the API, so the object store needs no browser-reachable address; `presigned` hands out short-lived URLs pointing at `S3_ENDPOINT`, which must then be reachable from your users' browsers |
